...
**`open-cli` turns OpenAPI descriptions into a remote-only, policy-aware command surface.** Point `open-cli` at a hosted runtime and an OpenAPI file, and it generates a typed CLI — with help text, parameter validation, secret resolution, policy enforcement, and audit logging — without writing glue code.

## Two binaries, two roles

| Binary | Role |
|--------|------|
| **`open-cli`** | User-facing client. Connects to the runtime, renders dynamic commands, and sends execution requests. |
...
`open-cli` always needs a reachable `open-cli-toolbox` runtime.

- For local evaluation, host `open-cli-toolbox` on your own machine and point `open-cli` at `http://127.0.0.1:8765`.
...
`open-cli` does **not** have a static command tree. It fetches the catalog before Cobra renders most help output. In practice that means even `open-cli --help` usually needs a reachable runtime plus a usable config path.
...
# In another shell:
open-cli --runtime http://127.0.0.1:8765 --config ./.cli.json --help
```

## How the pieces fit together

1. `open-cli` resolves a runtime URL from `--runtime`, `OPEN_CLI_RUNTIME_URL`, or `runtime.remote.url`.
...
5. `open-cli` renders service/group/tool commands from the selected effective view.
...
1. **[Installation](./installation)** — install `open-cli` and choose how you want to obtain the reference runtime (`open-cli-toolbox`).
