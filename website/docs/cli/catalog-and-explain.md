...
The safest way to understand an `open-cli` setup is to inspect the catalog before executing tools.
...
```bash
./bin/open-cli --runtime http://127.0.0.1:8765 --config ./.cli.json catalog list --format pretty
```
...
```bash
./bin/open-cli --runtime http://127.0.0.1:8765 --config ./.cli.json tool schema tickets:createTicket --format pretty
```
...
```bash
./bin/open-cli --runtime http://127.0.0.1:8765 --config ./.cli.json explain tickets:createTicket --format pretty
```
...
- a tool may be visible to `tool schema` but absent from `open-cli <service> <group> ...`
...
- command path: `open-cli helpdesk tickets list-tickets`
