...
```bash
open-cli helpdesk tickets get-ticket T-100
```
...
If a parameter has `x-cli-name`, that override becomes the flag name. Otherwise `open-cli` slugifies the original parameter name.
...
```bash
# inline JSON
./bin/open-cli ... create --body '{"title":"Printer jam"}'

# from a file
./bin/open-cli ... create --body @ticket.json

# from stdin
cat ticket.json | ./bin/open-cli ... create --body -
```
...
```bash
HELPDESK_TOKEN=token-abc ./bin/open-cli   --runtime http://127.0.0.1:8765   --config ./.cli.json   --approval   helpdesk tickets create   --body @ticket.json
```
...
- `open-cli --format json` prints the JSON body directly when possible
