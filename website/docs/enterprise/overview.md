...
This page organizes the evaluation material for the four audiences that typically review `oas-cli-go` before a pilot decision:
...
`oasclird` supports three practical deployment topologies.

| Model | Description | Reference |
|---|---|---|
| Embedded | Runtime runs in-process per invocation. No daemon. | [Deployment Models](../runtime/deployment-models) |
| Local daemon | Single `oasclird` process shared across CLI invocations. Warmed cache. | [Deployment Models](../runtime/deployment-models) |
| Remote runtime | Centrally hosted `oasclird` with network-controlled access. | [Deployment Models](../runtime/deployment-models) |
...
|---|---|---|
| CI-reproducible product paths | `make product-test-fleet` — artifacts written under `/tmp/oascli-fleet/` | [Fleet Validation](../development/fleet-validation) |
...
The following capabilities are **not provided by `oas-cli-go` itself**. They require operator-owned infrastructure or process. This list is consolidated here so reviewers do not have to infer it across pages.
...
| Network access control to `oasclird` | Bind address controls the interface. Beyond that, `oasclird` itself does not enforce network perimeter. | Firewall rules, reverse proxy auth, SSH tunneling, or container/network isolation for remote deployments. |
