...
const (
	...
	authentikPrimarySlug       = "open-cli-runtime"
	authentikAlternateSlug     = "open-cli-runtime-alt-issuer"
	authentikCallbackURL       = "http://127.0.0.1:8787/callback"
	authentikRunTestsEnv       = "OPEN_CLI_RUN_AUTHENTIK_TESTS"
	authentikClientIDEnv       = "OPEN_CLI_REMOTE_CLIENT_ID"
	authentikClientSecretEnv   = "OPEN_CLI_REMOTE_CLIENT_SECRET"
...
scope_mappings = [
    ensure_scope("open-cli runtime bundle:tickets", "bundle:tickets"),
    ensure_scope("open-cli runtime tool:tickets:listTickets", "tool:tickets:listTickets"),
    ensure_scope("open-cli runtime tool:users:listUsers", "tool:users:listUsers"),
    ensure_scope("open-cli runtime profile:wrong-audience", "profile:wrong-audience"),
]
...
result = {
    "primary": ensure_provider("open-cli Runtime Provider", "open-cli Runtime", %q),
    "alternate": ensure_provider("open-cli Runtime Provider Alt Issuer", "open-cli Runtime Alt Issuer", %q),
}
...
func resolveAuthentikFixtureAvailability(service, containerID string, enableTests bool) (bool, bool, string) {
	if !enableTests {
		return false, false, "live Authentik product tests are disabled; set OPEN_CLI_RUN_AUTHENTIK_TESTS=1 or use make test-runtime-auth-authentik"
	}
...
func runAuthentikManageScript(t *testing.T, workerID, script string) ([]byte, error) {
	...
	const marker = "__OPEN_CLI_JSON__="
